...
static public function insert(){try{ return Dbconnect::add('voyage_organises',"(NULL,?,?,?,?,?,?,?,?,?)",[self::$duree, self::$date_de_depart, self::$Prix, self::$discription, self::$food, self::$image,self::$etat,self::$ville,self::$id_hotel]); }catch(PDOException $e){ return $e->getMessage();}}
static  public  function Afficher(){return static :: SELECTJoin('voyage_organises,hotel','voyage_organises.id,voyage_organises.Dure,voyage_organises.date_de_depart,voyage_organises.Prix,voyage_organises.Discription,voyage_organises.Food,voyage_organises.image ,voyage_organises.etat,hotel.ville,hotel.etoile,hotel.id,hotel.name '," hotel.id = voyage_organises.id_Hotel");}
static  public  function selectOne(){return static :: SELECTJoin('voyage_organises,hotel','voyage_organises.id , voyage_organises.Dure,voyage_organises.Prix,voyage_organises.Discription,voyage_organises.image,voyage_organises.Food,hotel.ville,hotel.etoile',"  voyage_organises.id_Hotel=hotel.id and voyage_organises.id=4");}
